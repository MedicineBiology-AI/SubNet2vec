...
def sample():
    ...
    neg_file = r"E:\A_nature\result\negtive_drug_disease43.txt"
...
def main():
    ...
    sub_ht_ave,menche_ht_ave,phe_ht_ave=[],[],[]
    X, y = shengcheng()
    skf = KFold(n_splits=10, shuffle=True, random_state=1)
    for train_index, test_index in skf.split(X, y):
        X_train, X_test = X[train_index], X[test_index]
    ...
    print("phenoNet", "proximity", "subNet2vec")
    print(menche_ht_ave.mean(axis=0))
    print(phe_ht_ave.mean(axis=0))
    print(sub_ht_ave.mean(axis=0))
